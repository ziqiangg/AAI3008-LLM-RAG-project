...
async function loadSession(sessionId) {
  try {
    ...
    const sessionSources = lastSources ? (lastSources.chunks || lastSources) : [];
    if (sessionSources.length > 0) {
      hasActiveQuerySources = true;
      renderSources(sessionSources);
    } else {
...
function toggleRewriteDetails(messageId) {
  ...
  if (d && t) { const v = d.style.display !== 'none'; d.style.display = v ? 'none' : 'block'; t.textContent = v ? 'v' : '^'; }
}
